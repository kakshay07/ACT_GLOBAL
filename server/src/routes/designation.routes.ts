...
res) => {
  ...
  if(data){
    res.json(new ApiResponse(200,{},'Designation added succesfully'));
  }else{
...
designationRouter.put('/', async (req : AuthenticatedRequest<
  ...
  >, res) => {
  const {name , desig_id}=req.body;
  const {entity_id}=req.query;
  const response = await designation_model.updateDesignation({entity_id , name , desig_id , mo_by : req.user.user_id}); 
  if(response){
